...
class Review extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <Head>
          <title>{`Foodie - ${eatery.name}`}</title>
        </Head>
        <Header user={user}/>
                ...
                <div className={classes.section}>
                  <a target="_blank"
                    ...
                    <img className={classes.mapImage}
                         src={`https://maps.googleapis.com/maps/api/staticmap?center=${eatery.lat},${eatery.lng}&zoom=15&size=600x300&maptype=roadmap&markers=color:red|${eatery.lat},${eatery.lng}&key=${GOOGLE_API_KEY}`}/>
                  </a>
                </div>
              )}
              <Grid container
